...
def test_cli_help():
    with patch.object(sys, "argv", ["rocototop", "--help"]):
...
def test_cli_missing_args():
    with patch.object(sys, "argv", ["rocototop"]):
        with pytest.raises(SystemExit) as e:
            main()
        assert e.value.code != 0
...
def test_cli_nonexistent_files():
    with patch.object(sys, "argv", ["rocototop", "-w", "no.xml", "-d", "no.db"]):
        with pytest.raises(SystemExit) as e:
            main()
...
def test_cli_interval_parsing(tmp_path):
    ...
    with patch("rocototop.cli.RocotoApp") as mock_app:
        # Mock app.run() to avoid starting the TUI
        mock_app.return_value.run.return_value = None
        with patch.object(sys, "argv", ["rocototop", "-w", str(wf), "-d", str(db), "-i", "45"]):
            main()
