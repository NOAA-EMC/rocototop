...
<workflow name="test">
  ...
  <metatask name="ensemble">
    ...
    c.execute("CREATE TABLE IF NOT EXISTS jobs (taskname TEXT, cycle INTEGER, state TEXT, exit_status INTEGER, duration INTEGER, tries INTEGER, jobid TEXT)")
    c.execute("DELETE FROM jobs")
    c.execute("INSERT INTO jobs VALUES ('post_01', 1672531200, 'SUCCEEDED', 0, 100, 1, '12345')")
    c.execute("INSERT INTO jobs VALUES ('post_02', 1672531200, 'RUNNING', None, 50, 1, '12346')")
    conn.commit()
    conn.close()
    ...
    async with app.run_test() as pilot:
        ...
        # but here we just want to see if it runs and we can maybe convert it or just trust the tests.
